...
const login = () => {
    ...
    axios.post("http://localhost:8001/comes-bebes/login", {
        ...
        Swal.fire({
            icon: 'error',
            title: 'servidor não esta ligado!!',
            showConfirmButton: false,
            timer: 2000
        });
        console.log(error);
		...
		<div className="col-4 pt-5 mt-5">
			<div className="pt-5 mt-5 col-10">
                ...
                <div className="input-group m-2">
                    ...
                    <button className="input-group-text" onClick={handleShowPass}>{showPass ? <BsFillEyeSlashFill/> : <BsFillEyeFill/>}</button>
                </div>
                
				<button type="button" onClick={login} className="btn btn-success m-2 rounded-1">entrar</button>
			</div>
		</div>
    </div>
);
